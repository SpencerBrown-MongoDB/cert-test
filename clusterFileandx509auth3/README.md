This is like "clusterFileandx509auth" except that the certificateKeyFile has a DN that looks like one generated by Let's Encrypt or GTS, i.e. "CN=*.sub.domain". We use the tls config for attributes to specify that that attribute signifies internal cluster authentication.
